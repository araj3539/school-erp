...
export default function StudentFeeManagementPanel({ studentId, editable = true }: { studentId?: string; editable?: boolean }) {
  ...
  const [form, setForm] = useState({ headId: "", name: "", code: "", category: "tuition", amount: "", dueDate: "" });
  ...
  const feesQuery = useQuery({ queryKey: ["fees", "student", studentId], queryFn: async () => (await api.get(`/fees/student/${studentId}`)).data, enabled });
  ...
  const createHead = useMutation({
    mutationFn: () => api.post("/fees/heads", { name: form.name, code: form.code, category: form.category, kind: "recurring" }),
    onSuccess: async (response) => { const headId = response.data?.feeHead?._id; setForm((v) => ({ ...v, headId: headId || v.headId, name: "", code: "" })); setError(""); await queryClient.invalidateQueries({ queryKey: ["feeHeads"] }); },
    onError: (e: any) => setError(e?.response?.data?.message || "Unable to create fee head."),
  });
  const createItem = useMutation({
    mutationFn: async () => { if (!form.headId) throw new Error("Select a fee head first."); if (!fees[0]?._id) throw new Error("Generate or create a fee record for this student first."); return api.post("/fees/items", { studentId, feeId: fees[0]._id, feeHeadId: form.headId, label: heads.find((h: any) => h._id === form.headId)?.name || "Custom fee", amount: Number(form.amount), dueDate: form.dueDate ? new Date(`${form.dueDate}T00:00:00.000Z`).toISOString() : undefined }); },
  ...
  return <>
    ...
    <Modal isOpen={itemOpen} onClose={() => !createItem.isPending && !createHead.isPending && setItemOpen(false)} title="Add custom fee" size="md"><div className="space-y-4"><div className="rounded-xl bg-slate-50 p-3 text-xs text-slate-500">Student-specific fee items and adjustments are recorded in the audit log.</div><Select label="Fee head" value={form.headId} onChange={(e) => setForm((v) => ({ ...v, headId: e.target.value }))}><option value="">Select fee head</option>{heads.map((h: any) => <option key={h._id} value={h._id}>{h.name} · {h.category || h.kind}</option>)}</Select><div className="grid gap-3 sm:grid-cols-2"><Input label="Amount" type="number" min="0" value={form.amount} onChange={(e) => setForm((v) => ({ ...v, amount: e.target.value }))} /><Input label="Due date" type="date" value={form.dueDate} onChange={(e) => setForm((v) => ({ ...v, dueDate: e.target.value }))} /></div><div className="border-t border-slate-200 pt-4"><p className="mb-2 text-sm font-bold text-slate-900">Need a new fee head?</p><div className="grid gap-3 sm:grid-cols-2"><Input label="Name" placeholder="e.g. Annual activity" value={form.name} onChange={(e) => setForm((v) => ({ ...v, name: e.target.value }))} /><Input label="Code" placeholder="ACTIVITY" value={form.code} onChange={(e) => setForm((v) => ({ ...v, code: e.target.value.toUpperCase() }))} /><Select label="Category" value={form.category} onChange={(e) => setForm((v) => ({ ...v, category: e.target.value }))}>{categories.map((c) => <option key={c} value={c}>{c}</option>)}</Select><div className="flex items-end"><Button type="button" variant="outline" className="w-full" disabled={!form.name || !form.code || createHead.isPending} onClick={() => createHead.mutate()}>{createHead.isPending ? "Creating…" : "Create head"}</Button></div></div></div>{error && <p className="rounded-xl bg-rose-50 p-3 text-sm text-rose-700" role="alert">{error}</p>}<div className="flex justify-end gap-2"><Button variant="secondary" onClick={() => setItemOpen(false)}>Cancel</Button><Button disabled={!form.headId || !form.amount || createItem.isPending} onClick={() => createItem.mutate()}>{createItem.isPending ? "Adding…" : "Add fee"}</Button></div></div></Modal>
    <Modal isOpen={Boolean(adjustTarget)} onClose={() => !adjustItem.isPending && setAdjustTarget(null)} title="Adjust student fee" size="md"><div className="space-y-4"><p className="text-sm text-slate-500">Discounts, waivers, surcharges and amount overrides require a reason and are audited. Paid/partially paid items remain protected.</p><Select label="Adjustment" value={adjustment.type} onChange={(e) => setAdjustment((v) => ({ ...v, type: e.target.value as Adjustment }))}><option value="discount">Discount</option><option value="waiver">Waiver</option><option value="surcharge">Surcharge</option><option value="amount_override">Amount override</option></Select><Input label="Amount" type="number" min="0" value={adjustment.amount} onChange={(e) => setAdjustment((v) => ({ ...v, amount: e.target.value }))} /><Input label="Reason (required)" value={adjustment.reason} onChange={(e) => setAdjustment((v) => ({ ...v, reason: e.target.value }))} placeholder="e.g. sibling concession" />{error && <p className="rounded-xl bg-rose-50 p-3 text-sm text-rose-700" role="alert">{error}</p>}<div className="flex justify-end gap-2"><Button variant="secondary" onClick={() => setAdjustTarget(null)}>Cancel</Button><Button disabled={!adjustment.amount || !adjustment.reason.trim() || adjustItem.isPending} onClick={() => adjustItem.mutate()}>{adjustItem.isPending ? "Saving…" : "Save adjustment"}</Button></div></div></Modal>
  </>;
}
