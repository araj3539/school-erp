...
export function Toaster({ toasts, onClose }: ToasterProps) {
  return (
    <div aria-live="polite" aria-atomic="false" className="pointer-events-none fixed bottom-4 right-4 z-[60] flex w-[calc(100vw-2rem)] max-w-md flex-col gap-2 sm:w-auto sm:min-w-[320px]">
      {toasts.map((toast) => (
        <div key={toast.id} role={toast.type === "error" ? "alert" : "status"} className={cn("pointer-events-auto flex items-start gap-3 rounded-2xl border px-4 py-3.5 shadow-[0_14px_36px_rgba(15,23,42,0.12)]", "animate-slide-in", styles[toast.type])}>
