...
import { ClipboardList, LayoutDashboard, LogOut, Menu, Users, X, BriefcaseBusiness, HeartHandshake, CalendarCheck2, Award, CreditCard, CalendarClock, Bell } from "lucide-react";
...
const PORTAL_NAV: PortalNavItem[] = [
  ...
  { label: "Fees", path: "/portal-fees", icon: <CreditCard className="h-5 w-5" aria-hidden="true" />, permissions: ["fees:read:own", "fees:read:child"], roles: ["student", "parent"] },
  { label: "Timetable", path: "/portal-timetable", icon: <CalendarClock className="h-5 w-5" aria-hidden="true" />, permissions: ["timetable:read:own", "timetable:read:child"], roles: ["teacher", "student", "parent"] },
  { label: "Notices", path: "/portal-notices", icon: <Bell className="h-5 w-5" aria-hidden="true" />, permissions: ["notices:read"], roles: ["teacher", "student", "parent"] },
...
export function PortalLayout() {
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-900">
    ...
    <aside className={cn("fixed inset-y-0 left-0 z-50 w-72 border-r border-slate-200 bg-white transition-transform duration-200 lg:translate-x-0", open ? "translate-x-0" : "-translate-x-full")}><div className="flex h-full flex-col">
      ...
      <nav id="portal-navigation" aria-label="Portal navigation" className="flex-1 space-y-1 overflow-y-auto p-4">{items.map((item) => { const active = location.pathname === item.path || (item.path !== "/dashboard" && location.pathname.startsWith(`${item.path}/`)); return <NavLink key={`${item.path}-${item.label}`} to={item.path} aria-current={active ? "page" : undefined} className={cn("flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-medium transition-colors focus:outline-none focus-visible:ring-2 focus:ring-primary-500", active ? "bg-primary-50 text-primary-700" : "text-slate-600 hover:bg-slate-100 hover:text-slate-900")}>{item.icon}<span>{item.label}</span></NavLink>; })}</nav>
