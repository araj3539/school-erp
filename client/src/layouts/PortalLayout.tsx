  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <aside className={cn("fixed inset-y-0 left-0 z-50 w-72 border-r border-slate-200 bg-white transition-transform duration-200 lg:translate-x-0", open ? "translate-x-0" : "-translate-x-full")}>
        <div className="flex h-full flex-col">
          <div className="flex h-16 items-center justify-between border-b border-slate-200 px-5">
            ...
            <button type="button" onClick={() => setOpen(false)} aria-label="Close navigation menu" className="rounded-lg p-2 text-slate-500 hover:bg-slate-100 focus:outline-none focus-visible:ring-2 focus:ring-primary-500 lg:hidden"><X className="h-5 w-5" aria-hidden="true" /></button>
          </div>
      ...
      <div className="min-h-screen lg:pl-72">
        ...
        <main id="portal-main" tabIndex={-1} className="mx-auto w-full max-w-7xl p-4 sm:p-6 lg:p-8 focus:outline-none"><Suspense fallback={<PageLoader />}><Outlet /></Suspense></main>
      </div>
    </div>
  );
}
