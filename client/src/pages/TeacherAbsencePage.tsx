...
export default function TeacherAbsencePage() {
  ...
  const load = async () => { setLoading(true); try { const r = await api.get(`/teacher-absences?date=${date}`); setAbsences(r.data.data || []); } finally { setLoading(false); } };
  useEffect(() => { load(); }, [date]);
  const open = async (absence: Absence) => { const r = await api.get(`/teacher-absences/${absence._id}`); setSelected(r.data); };
  ...
  return <div className="space-y-6 p-4 md:p-6">
    ...
    <section className="rounded-xl border p-4 space-y-3"><div className="flex flex-col gap-2 sm:flex-row"><input className="rounded border px-3 py-2" type="date" value={date} onChange={e => setDate(e.target.value)} /><input className="rounded border px-3 py-2 flex-1" placeholder="Teacher ID" value={teacherId} onChange={e => setTeacherId(e.target.value)} /><button className="rounded bg-primary px-4 py-2 text-primary-foreground" onClick={report} disabled={!teacherId}>Report absence</button></div></section>
