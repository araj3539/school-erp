...
export default function PortalFeesPage() {
  ...
  const students = data?.students || []; const student = students[0]; const fees = data?.fees || []; const summary = data?.summary || { totalDue: 0, paid: 0, balance: 0, overdue: 0 };
  return <div className="space-y-6">
    ...
    {isParent && students.length > 1 && <Card><CardHeader><h2 className="font-semibold">Selected child</h2></CardHeader><CardContent><label className="sr-only" htmlFor="fees-child">Choose a child</label><select id="fees-child" value={student?._id || ""} onChange={(e) => setParams(e.target.value ? { childId: e.target.value } : {})} className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-800 outline-none focus:border-primary-500 focus:ring-2 focus:ring-primary-500 sm:max-w-md">{students.map((item: any) => <option key={item._id} value={item._id}>{item.firstName} {item.lastName} · {item.class}{item.section ? ` · ${item.section}` : ""}</option>)}</select></CardContent></Card>}
