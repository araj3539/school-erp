...
function ErrorState({ onRetry }: { onRetry: () => void }) {
  return (
    <div className="rounded-xl border border-red-200 bg-red-50 p-5 text-sm text-red-800">
      <p className="font-semibold">We couldn't load this report.</p>
      <p className="mt-1 text-red-700">Refresh the report and try again. Your report filters have been kept.</p>
...
export default function ReportsPage() {
  ...
  const studentsQuery = useQuery({
    queryKey: ["reports", "students", page, studentSearch],
    queryFn: async () => {
  ...
  const attendanceQuery = useQuery({
    queryKey: ["reports", "attendance", page, dateRange],
    queryFn: async () => {
      const params = new URLSearchParams({ page: page.toString(), limit: "20" });
  ...
  const feesQuery = useQuery({
    queryKey: ["reports", "fees", page, feeSearch],
    queryFn: async () => {
      const params = new URLSearchParams({ page: page.toString(), limit: "20" });
      if (feeSearch) params.append("search", feeSearch);
  ...
  const attendanceSummary = useMemo(() => {
    const records = attendance.flatMap((item: any) => item.records || []);
  ...
  const feeSummary = useMemo(() => {
    const totalDue = fees.reduce((sum: number, item: any) => sum + Number(item.totalDue || 0), 0);
    const paid = fees.reduce((sum: number, item: any) => sum + Number(item.paidAmount || 0), 0);
    return { totalDue, paid, balance: Math.max(0, totalDue - paid) };
  }, [fees]);
  ...
  const attendanceColumns = [
    ...
    { key: "classId", header: "Class", render: (a: any) => a.classId?.displayName || "-" },
    { key: "sectionId", header: "Section", render: (a: any) => a.sectionId?.name || "-" },
    { key: "total", header: "Total", render: (a: any) => a.records?.length || 0 },
    ...
    return (
      <div className="flex flex-col gap-3 border-t border-slate-100 pt-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button variant="outline" size="sm" onClick={() => setPage((current) => Math.min(pagination.totalPages, current + 1))} disabled={page === pagination.totalPages}>Next</Button>
        </div>
      </div>
    );
  };

  const exportReport = (type: string, format: "excel" | "pdf") => {
    ...
    window.open(`/api/reports/${type}?${query.toString()}`, "_blank");
  };
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-2xl border border-slate-200 bg-gradient-to-br from-white via-white to-slate-50 p-5 shadow-sm sm:p-6">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <div className="mb-3 flex h-10 w-10 items-center justify-center rounded-xl bg-primary-50 text-primary-600">
              <FileBarChart3 className="h-5 w-5" aria-hidden="true" />
            </div>
            <p className="text-sm font-semibold uppercase tracking-wider text-primary-600">Insights & reporting</p>
            <h1 className="mt-1 text-2xl font-bold tracking-tight text-slate-950 sm:text-3xl">Reports</h1>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">Turn student, attendance, and fee records into focused operational views. Filter the register you need, then export it for sharing or follow-up.</p>
          </div>
          <div className="flex items-center gap-2 text-xs text-slate-500"><Calendar className="h-4 w-4" aria-hidden="true" /> Live report data</div>
        </div>
      </section>
          ...
          <Card key={label} className="overflow-hidden">
            <CardContent className="p-5">
              <div className="flex items-start justify-between gap-3">
                <div><p className="text-sm font-medium text-slate-500">{label}</p><p className="mt-2 text-2xl font-bold tracking-tight text-slate-950">{value}</p><p className="mt-1 text-xs text-slate-500">{hint}</p></div>
                <div className="rounded-xl bg-slate-100 p-2.5 text-slate-600"><Icon className="h-5 w-5" aria-hidden="true" /></div>
              </div>
            </CardContent>
          </Card>
        ))}
      ...
      <Tabs value={activeTab} onValueChange={(value) => { setActiveTab(value); setPage(1); }} className="space-y-4">
        <TabsList className="grid h-auto w-full grid-cols-3 rounded-xl bg-slate-100 p-1 sm:flex sm:w-fit">
          {reportTabs.map(({ value, label, icon: Icon }) => <TabsTrigger key={value} value={value} className="rounded-lg px-3 py-2.5"><Icon className="mr-2 h-4 w-4" aria-hidden="true" />{label}</TabsTrigger>)}
        </TabsList>
