...
export default function PortalResultsPage() {
  ...
  return <div className="space-y-6">
    ...
    {isParent && students.length > 1 && <Card><CardHeader><h2 className="font-semibold">Selected child</h2></CardHeader><CardContent><label className="sr-only" htmlFor="results-child">Choose a child</label><select id="results-child" value={selectedStudent?._id || ""} onChange={(e) => setParams(e.target.value ? { childId: e.target.value } : {})} className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-800 outline-none focus:border-primary-500 focus:ring-2 focus:ring-primary-500 sm:max-w-md">{students.map((student: any) => <option key={student._id} value={student._id}>{student.firstName} {student.lastName} · {student.class}{student.section ? ` · ${student.section}` : ""}</option>)}</select></CardContent></Card>}
