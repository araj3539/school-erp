...
export default function PortalResultsPage() {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ["portal-results", user?.role, childId],
    queryFn: async () => (await api.get(`/portal/results${isParent && childId ? `?childId=${encodeURIComponent(childId)}` : ""}`)).data,
  });
  ...
  return <div className="space-y-6">
    ...
    {isParent && students.length > 1 && <Card><CardHeader><h2 className="font-semibold">Selected child</h2></CardHeader><CardContent><p className="font-medium">{selectedStudent?.firstName} {selectedStudent?.lastName}</p><p className="text-sm text-slate-500">{selectedStudent?.class}{selectedStudent?.section ? ` · ${selectedStudent.section}` : ""}</p></CardContent></Card>}
