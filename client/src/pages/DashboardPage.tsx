...
interface StatCardProps { title: string; value: string | number; icon: React.ReactNode; tone: string; isLoading?: boolean; }
function StatCard({ title, value, icon, tone, isLoading }: StatCardProps) {
  return (
    <Card className="group relative overflow-hidden transition-[border-color,box-shadow,transform] duration-200 hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-[0_14px_32px_rgba(15,23,42,0.07)]">
      <span className="absolute inset-x-0 top-0 h-0.5 bg-gradient-to-r from-transparent via-primary-500/70 to-transparent opacity-0 transition-opacity duration-200 group-hover:opacity-100" aria-hidden="true" />
      <CardContent className="p-5 sm:p-6">
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            <p className="text-[11px] font-bold uppercase tracking-[0.12em] text-slate-500">{title}</p>
            {isLoading ? <div className="mt-3 h-9 w-24 animate-pulse rounded-lg bg-slate-200" aria-hidden="true" /> : <p className="mt-2 truncate text-2xl font-extrabold tracking-[-0.03em] text-slate-950 sm:text-[28px]">{value}</p>}
            {!isLoading && <div className="mt-2 flex items-center gap-1.5 text-[11px] font-semibold text-slate-400"><ArrowUpRight className="h-3.5 w-3.5 text-emerald-500" aria-hidden="true" />Current snapshot</div>}
          </div>
          <div className={cn("flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl ring-1 ring-inset ring-black/[0.03]", tone)} aria-hidden="true">{icon}</div>
        </div>
      </CardContent>
    </Card>
  );
...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-6 lg:space-y-8">
      <header className="relative overflow-hidden rounded-3xl border border-slate-200/80 bg-white px-5 py-6 shadow-[0_1px_2px_rgba(15,23,42,0.03)] sm:px-7 sm:py-7">
        <div className="pointer-events-none absolute -right-20 -top-24 h-56 w-56 rounded-full bg-primary-50 blur-3xl" aria-hidden="true" />
        <div className="relative flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <div className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.16em] text-primary-600"><span className="h-1.5 w-1.5 rounded-full bg-primary-500" aria-hidden="true" />Overview</div>
            <h1 className="mt-2 text-2xl font-extrabold tracking-[-0.035em] text-slate-950 sm:text-3xl">School dashboard</h1>
            <p className="mt-1.5 max-w-xl text-sm leading-6 text-slate-500">A focused view of today’s school operations, with the signals that deserve your attention first.</p>
          </div>
      ...
      <section aria-label="Recent admissions">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between gap-3 border-b border-slate-100/90">
            <div><div className="flex items-center gap-2"><h2 className="text-sm font-bold text-slate-900">Recent admissions</h2><Badge variant="secondary">{recentAdmissions.length}</Badge></div><p className="mt-1 text-xs text-slate-500">The latest students added to the school</p></div>
            <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-primary-50 text-primary-600" aria-hidden="true"><UserRound className="h-4 w-4" /></div>
          </CardHeader>
          <CardContent className="pt-4">
            {statsLoading ? <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-5"><div className="h-14 animate-pulse rounded-xl bg-slate-100" /><div className="h-14 animate-pulse rounded-xl bg-slate-100" /><div className="h-14 animate-pulse rounded-xl bg-slate-100" /></div> : recentAdmissions.length === 0 ? <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50/70 px-4 py-8 text-center text-sm font-medium text-slate-500">No admissions recorded yet</div> : <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-5">{recentAdmissions.map((student: any) => <div key={student._id} className="group flex min-w-0 items-center gap-3 rounded-xl border border-slate-100 bg-slate-50/60 p-3 transition-[border-color,background-color,box-shadow] hover:border-slate-200 hover:bg-white hover:shadow-sm"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-primary-50 text-xs font-bold text-primary-700" aria-hidden="true">{student.firstName?.charAt(0)}{student.lastName?.charAt(0)}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold text-slate-800">{student.firstName} {student.lastName}</p><p className="truncate text-xs text-slate-500">{student.classId?.displayName || "Class not assigned"}</p></div><Badge variant="secondary">{student.admissionNo}</Badge></div>)}</div>}
          </CardContent>
        </Card>
      </section>
      ...
      <section className="grid gap-5 lg:grid-cols-2" aria-label="Trends">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between gap-3">
            ...
            <div className="text-right"><p className="text-sm font-bold tabular-nums text-slate-900">{latestAttendance != null ? `${latestAttendance}%` : "—"}</p><p className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">Latest</p></div>
          </CardHeader>
      ...
      <section className="grid gap-5 lg:grid-cols-2">
        <Card>
          ...
          <CardContent>{chartsLoading ? <p className="text-sm text-slate-500">Loading fee status...</p> : feeStatus.length === 0 ? <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50/70 px-4 py-8 text-center text-sm font-medium text-slate-500">No fee records yet</div> : <ul className="divide-y divide-slate-100">{feeStatus.map((fs: any) => <li key={fs._id} className="flex items-center justify-between gap-4 py-3 first:pt-0 last:pb-0"><span className="text-sm font-semibold capitalize text-slate-700">{String(fs._id).replace(/_/g, " ")}</span><Badge variant={feeStatusVariant(fs._id)}>{fs.count} · {formatCurrency(fs.total || 0)}</Badge></li>)}</ul>}</CardContent>
        </Card>
        <Card>
