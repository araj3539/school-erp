...
function calendarDayStart(value: Date): Date {
...
async function assertAttendanceDateInCurrentAcademicYear(schoolId: Types.ObjectId, date: Date) {
  ...
  const startDay = calendarDayStart(academicYear.startDate);
  const endDay = calendarDayStart(academicYear.endDate);
  if (date < startDay || date >= endDay) throw AppError.badRequest("Attendance date must fall within the current academic year");
}
async function assertAttendanceCorrectionAccess(req: Request) {
  if (req.user!.role !== UserRole.PRINCIPAL && req.user!.role !== UserRole.SUPER_ADMIN) throw AppError.forbidden("Only school management can correct existing attendance");
}
async function validateAttendanceEntry(req: Request, entry: any, session: mongoose.ClientSession) {
  const schoolId = getTenantId(req);
  const date = parseCalendarDate(String(entry.date));
  ...
  const [cls, section] = await Promise.all([
    Class.findOne({ _id: entry.classId, schoolId }).select("_id").session(session),
    Section.findOne({ _id: entry.sectionId, classId: entry.classId, schoolId }).select("_id").session(session),
  ]);
  if (!cls || !section) throw AppError.notFound("Class or section not found");
  const studentIds = entry.records.map((record: any) => record.studentId);
  ...
  const studentCount = await Student.countDocuments({ _id: { $in: studentIds }, classId: entry.classId, sectionId: entry.sectionId, schoolId, status: "active" }).session(session);
  ...
  return { schoolId, date, records: entry.records.map((record: any) => ({ ...record, studentId: new Types.ObjectId(record.studentId) })) };
}
...
export async function getAttendance(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (filters.date) { const date = parseCalendarDate(String(filters.date)); dbQuery.date = { $gte: date, $lt: addCalendarDays(date, 1) }; }
    if (filters.startDate || filters.endDate) { dbQuery.date = {}; if (filters.startDate) dbQuery.date.$gte = parseCalendarDate(String(filters.startDate)); if (filters.endDate) dbQuery.date.$lt = addCalendarDays(parseCalendarDate(String(filters.endDate)), 1); }
    const sort: any = {}; if (sortBy) sort[sortBy] = sortOrder === "asc" ? 1 : -1; else sort.date = -1;
    const skip = (page - 1) * limit;
    const [attendance, total] = await Promise.all([Attendance.find(dbQuery).populate("classId sectionId markedBy").sort(sort).skip(skip).limit(limit).lean(), Attendance.countDocuments(dbQuery)]);
...
export async function markAttendance(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const [cls, section] = await Promise.all([Class.findOne({ _id: data.classId, schoolId }).select("_id"), Section.findOne({ _id: data.sectionId, classId: data.classId, schoolId }).select("_id")]);
    if (!cls || !section) throw AppError.notFound("Class or section not found");
...
export async function bulkMarkAttendance(req: Request, res: Response, next: NextFunction) {
  ...
  try {
    ...
    await session.withTransaction(async () => {
      for (const item of prepared) {
        ...
        let attendance = await Attendance.findOne({ date, classId: entry.classId, sectionId: entry.sectionId, schoolId }).session(session);
        const corrected = Boolean(attendance);
        if (attendance) {
          await assertAttendanceCorrectionAccess(req);
...
export async function getStudentAttendance(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const schoolId = getTenantId(req);
    await assertStudentOwnAccess(req, id); await assertParentChildAccess(req, id);
    ...
    if (startDate || endDate) { dbQuery.date = {}; if (startDate) dbQuery.date.$gte = parseCalendarDate(String(startDate)); if (endDate) dbQuery.date.$lt = addCalendarDays(parseCalendarDate(String(endDate)), 1); }
    ...
    attendance.forEach((a) => { const record = a.records.find((r) => r.studentId.toString() === id); if (record) summary[record.status as keyof typeof summary]++; });
...
export async function getMonthlyAttendanceReport(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (!academicYear) throw AppError.conflict("Current academic year could not be resolved");
    const academicStart = calendarDayStart(academicYear.startDate);
    const academicEnd = calendarDayStart(academicYear.endDate);
    if (startDate < academicStart || startDate >= academicEnd) throw AppError.badRequest("Attendance report must fall within the current academic year");
    ...
    const report = students.map((student: any) => { const studentAttendance = attendance.map((a) => a.records.find((r) => r.studentId.toString() === student._id.toString())?.status ?? null); const counts = { present: 0, absent: 0, late: 0, halfDay: 0, onLeave: 0 }; studentAttendance.forEach((status) => { if (status) counts[status as keyof typeof counts]++; }); return { studentId: student._id, admissionNo: student.admissionNo, name: `${student.firstName} ${student.lastName}`, attendance: studentAttendance, counts }; });
