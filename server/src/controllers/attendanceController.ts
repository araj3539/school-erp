...
export async function getAttendance(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (filters.date) { const date = new Date(filters.date as string); const nextDate = new Date(date); nextDate.setDate(date.getDate() + 1); dbQuery.date = { $gte: date, $lt: nextDate }; }
    if (filters.startDate || filters.endDate) { dbQuery.date = {}; if (filters.startDate) dbQuery.date.$gte = new Date(filters.startDate as string); if (filters.endDate) dbQuery.date.$lte = new Date(filters.endDate as string); }
...
export async function markAttendance(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (existing) { existing.records = records; existing.markedBy = new Types.ObjectId(req.user!.userId); await existing.save(); await createAuditLog({ userId: req.user!.userId, action: "UPDATE", entity: "Attendance", entityId: existing._id.toString(), after: { recordsCount: records.length } }); return res.json({ attendance: existing }); }
...
export async function getStudentAttendance(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (startDate || endDate) { dbQuery.date = {}; if (startDate) dbQuery.date.$gte = new Date(startDate); if (endDate) dbQuery.date.$lte = new Date(endDate); }
...
export async function getMonthlyAttendanceReport(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const startDate = new Date(Number(year), Number(month) - 1, 1); const endDate = new Date(Number(year), Number(month), 0, 23, 59, 59); const schoolId = req.user!.schoolId;
    const section = await Section.findOne({ _id: sectionId, classId, schoolId }).select("_id"); if (!section) throw AppError.notFound("Class or section not found");
    const students = await Student.find({ classId, sectionId, schoolId, status: "active" }).lean(); const attendance = await Attendance.find({ classId, sectionId, schoolId, date: { $gte: startDate, $lte: endDate } }).lean();
