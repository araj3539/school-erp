...
const approvalBody = z.object({ proposedChanges: z.record(z.unknown()) });
const reviewBody = z.object({ approved: z.boolean(), reason: z.string().trim().max(500).optional() });
const orderBody = z.object({ orderId: z.string().min(1) });
...
export async function reviewFeePolicy(req: Request, res: Response, next: NextFunction) { try { const data = reviewBody.parse(req.body); res.json({ approval: await reviewFeePolicyApproval(school(req), req.params.id, data.approved, req.user!.userId, data.reason) }); } catch (e) { next(e); } }
