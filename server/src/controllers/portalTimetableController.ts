...
async function resolveScope(req: Request) {
  ...
  if (req.user!.role === UserRole.STUDENT) {
    ...
    return { students: [student], classIds: student.classId ? [student.classId._id || student.classId] : [], sectionPairs: student.sectionId ? [{ classId: student.classId?._id || student.classId, sectionId: student.sectionId._id || student.sectionId }] : [] };
  }
  if (req.user!.role === UserRole.PARENT) {
    ...
    const students = await Student.find(filter).select("_id firstName lastName admissionNo classId sectionId").populate("classId sectionId").sort({ firstName: 1, lastName: 1 }).lean();
    if (!students.length) throw AppError.notFound(childId ? "Child not found" : "No linked children found");
    return { students, classIds: students.filter((s: any) => s.classId).map((s: any) => s.classId._id || s.classId), sectionPairs: students.filter((s: any) => s.classId && s.sectionId).map((s: any) => ({ classId: s.classId._id || s.classId, sectionId: s.sectionId._id || s.sectionId })) };
  }
  if (req.user!.role === UserRole.TEACHER) {
    ...
    return { students: [], teacherId: teacher._id, classIds: [], sectionPairs: [] };
  }
...
export async function getPortalTimetable(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (scope.teacherId) filter.teacherId = scope.teacherId;
      ...
      filter.$or = scope.sectionPairs.flatMap((pair: any) => [{ classId: pair.classId, sectionId: pair.sectionId }, { classId: pair.classId, sectionId: { $exists: false } }]);
    }
    ...
    res.json({ academicYear, students: scope.students.map((student: any) => ({ _id: student._id, firstName: student.firstName, lastName: student.lastName, class: student.classId?.displayName || "", section: student.sectionId?.name || "" })), entries: entries.map((item: any) => ({ _id: item._id, dayOfWeek: item.dayOfWeek, startTime: item.startTime, endTime: item.endTime, periodLabel: item.periodLabel || "", roomNumber: item.roomNumber || "", subject: item.subjectId?.name || "Subject", teacher: item.teacherId ? `${item.teacherId.firstName || ""} ${item.teacherId.lastName || ""}`.trim() : "", class: item.classId?.displayName || "Class", section: item.sectionId?.name || "" })) });
