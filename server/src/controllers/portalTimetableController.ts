...
async function resolveScope(req: Request) {
  ...
  if (req.user!.role === UserRole.STUDENT) {
    ...
    return { students: [student], classIds: student.classId ? [student.classId._id || student.classId] : [] };
  }
  if (req.user!.role === UserRole.PARENT) {
    ...
    return { students, classIds: students.filter((s: any) => s.classId).map((s: any) => s.classId._id || s.classId) };
  }
  if (req.user!.role === UserRole.TEACHER) {
    ...
    return { students: [], teacherId: teacher._id, classIds: [] };
  }
...
export async function getPortalTimetable(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (scope.teacherId) filter.teacherId = scope.teacherId;
      ...
      if (!student.classId) return res.json({ academicYear, students: [], entries: [] });
      ...
      if (!scope.classIds.length) return res.json({ academicYear, students: [], entries: [] });
