...
async function assertNoAcademicYearOverlap(schoolId: string, startDate: Date, endDate: Date, excludeId?: string, session?: mongoose.ClientSession) {
  const query: any = { schoolId, startDate: { $lt: endDate }, endDate: { $gt: startDate } };
  if (excludeId) query._id = { $ne: excludeId };
  const existing = await AcademicYear.findOne(query).session(session ?? null).select("_id name").lean();
  if (existing) throw AppError.conflict(`Academic year overlaps existing year ${existing.name}`);
}
...
export async function createAcademicYear(req: Request, res: Response, next: NextFunction) {
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      await assertNoAcademicYearOverlap(schoolId, start, end, undefined, session);
...
export async function updateAcademicYear(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    await assertNoAcademicYearOverlap(schoolId, nextStart, nextEnd, id);
