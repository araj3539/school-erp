...
function deletionDate(): Date {
  const date = new Date();
  date.setDate(date.getDate() + DOCUMENT_GRACE_PERIOD_DAYS);
  return date;
}

function isActiveDocument(document: any): boolean {
  return document.status !== "pending_deletion";
}

function activeDocuments(documents: any[]): any[] {
  return documents.filter(isActiveDocument);
}
...
export async function getStudents(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const safeStudents = students.map((student: any) => ({ ...student, documents: activeDocuments(student.documents || []) }));
    res.json({ data: safeStudents, pagination: { page, limit, total, totalPages: Math.ceil(total / limit) } });
...
export async function getStudentById(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const payload = student.toObject({ virtuals: true });
    payload.documents = activeDocuments(payload.documents || []);
...
export async function getStudentDocumentUrl(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (!document || !isActiveDocument(document)) throw AppError.notFound("Document not found");
...
export async function uploadStudentDocument(req: MulterRequest, res: Response, next: NextFunction) {
  ...
  try {
    ...
    const now = new Date();
    const previousPhoto = type === DocumentType.PHOTO
      ? student.documents.find((document: any) => document.type === DocumentType.PHOTO && isActiveDocument(document))
      : undefined;

    if (previousPhoto) {
      previousPhoto.status = "pending_deletion";
      previousPhoto.deletedAt = now;
      previousPhoto.deletionScheduledAt = deletionDate();
    }
    ...
    await createAuditLog({
      ...
      after: {
        ...
        mimeType: prepared.contentType,
        sizeBytes: prepared.buffer.length,
        replacedKey: previousPhoto?.url || undefined,
        previousDeletionScheduledAt: previousPhoto?.deletionScheduledAt || undefined
      }
    });
...
export async function deleteStudentDocument(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    document.deletionScheduledAt = deletionDate();
    await student.save();
    ...
    await createAuditLog({
      ...
      entity: "Student",
      entityId: student._id.toString(),
      after: { documentId, type: document.type, key: document.url, deletionScheduledAt: document.deletionScheduledAt }
    });

    res.json({ message: "Document scheduled for deletion", deletionScheduledAt: document.deletionScheduledAt });
  } catch (error) { next(error); }
}

export async function restoreStudentDocument(req: Request, res: Response, next: NextFunction) {
  try {
    const { id, documentId } = req.validatedParams as any;
    const student = await Student.findOne({ _id: id, schoolId: getTenantId(req) });
    if (!student) throw AppError.notFound("Student not found");
    const document = student.documents.find((item: any) => item._id?.toString() === documentId);
    if (!document || document.status !== "pending_deletion") throw AppError.notFound("Deleted document not found or grace period has ended");

    if (document.type === DocumentType.PHOTO) {
      const activePhoto = student.documents.find((item: any) => item.type === DocumentType.PHOTO && isActiveDocument(item));
      if (activePhoto) throw AppError.conflict("A student already has an active photo. Delete the current photo before restoring this one.");
    }

    document.status = "active";
    document.deletedAt = undefined;
    document.deletionScheduledAt = undefined;
    await student.save();
    ...
    await createAuditLog({
      ...
      entity: "Student",
      entityId: student._id.toString(),
      after: { documentId, type: document.type, key: document.url }
    });
