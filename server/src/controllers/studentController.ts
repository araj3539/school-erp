...
function documentStorageKey(schoolId: string, studentId: string, type: string): string {
  return buildR2Key(["schools", schoolId, "students", studentId, "documents", type]);
}
...
export async function getStudentById(req: Request, res: Response, next: NextFunction) {
  try { const { id } = req.validatedParams as any; const student = await Student.findOne({ _id: id, schoolId: getTenantId(req) }).populate("classId sectionId userId"); if (!student) throw AppError.notFound("Student not found"); res.json({ student }); }
  catch (error) { next(error); }
}
...
export async function updateStudent(req: Request, res: Response, next: NextFunction) {
  try { const { id } = req.validatedParams as any; const rawData = req.validatedBody as any; const { schoolId: _ignored, ...data } = rawData; const student = await Student.findOneAndUpdate({ _id: id, schoolId: getTenantId(req) }, data, { new: true }); if (!student) throw AppError.notFound("Student not found"); await createAuditLog({ userId: req.user!.userId, action: "UPDATE", entity: "Student", entityId: student._id.toString(), after: data }); res.json({ student }); }
  catch (error) { next(error); }
}

export async function deleteStudent(req: Request, res: Response, next: NextFunction) {
  try { const { id } = req.validatedParams as any; const student = await Student.findOneAndUpdate({ _id: id, schoolId: getTenantId(req) }, { status: "left" }, { new: true }); if (!student) throw AppError.notFound("Student not found"); await createAuditLog({ userId: req.user!.userId, action: "DELETE", entity: "Student", entityId: student._id.toString() }); res.json({ message: "Student marked as left" }); }
  catch (error) { next(error); }
}
...
export async function uploadStudentDocument(req: MulterRequest, res: Response, next: NextFunction) {
  ...
  try {
    const { id } = req.validatedParams as any; const schoolId = getTenantId(req); const type = String(req.body?.type || "").trim().toLowerCase();
