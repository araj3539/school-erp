...
function publicDocument(document: any) {
  ...
  return { _id: document._id, type: document.type, originalName: document.originalName, mimeType: document.mimeType, sizeBytes: document.sizeBytes, uploadedAt: document.uploadedAt };
}

function publicRecovery(item: any, now = new Date()) {
  return { _id: item._id, studentId: item.studentId, documentType: item.documentType, originalName: item.originalName, mimeType: item.mimeType, sizeBytes: item.sizeBytes, deletedAt: item.deletedAt, expiresAt: item.expiresAt, source: item.source, status: item.status, restoredAt: item.restoredAt, restoredBy: item.restoredBy, createdAt: item.createdAt, updatedAt: item.updatedAt, recoverable: item.status === "available" && new Date(item.expiresAt) > now };
}
...
export async function getStudents(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (req.user!.role === UserRole.STUDENT) dbQuery.userId = req.user!.userId;
    else if (req.user!.role === UserRole.PARENT) dbQuery.parentIds = req.user!.userId;
    else if (req.user!.role === UserRole.TEACHER) {
      const classIds = await getTeacherClassIds(req);
      if (!classIds?.length) { res.json({ data: [], pagination: { page, limit, total: 0, totalPages: 0 } }); return; }
    ...
    if (filters.search && req.user!.role !== UserRole.STUDENT) { const search = escapeRegex(String(filters.search)); dbQuery.$or = [{ firstName: { $regex: search, $options: "i" } }, { lastName: { $regex: search, $options: "i" } }, { admissionNo: { $regex: search, $options: "i" } }, { phone: { $regex: search, $options: "i" } }]; }
    const sort: any = {};
...
export async function getStudentById(req: Request, res: Response, next: NextFunction) {
  try { const { id } = req.validatedParams as any; await assertStudentReadAccess(req, id); const student = await Student.findOne({ _id: id, schoolId: getTenantId(req) }).select("-documents.url -documents.publicId").populate("classId sectionId userId"); if (!student) throw AppError.notFound("Student not found"); res.json({ student }); } catch (error) { next(error); }
}
...
export async function uploadStudentDocument(req: MulterRequest, res: Response, next: NextFunction) { try { const { id } = req.validatedParams as any; const schoolId = getTenantId(req); const type = String(req.body?.type || "").trim().toLowerCase(); if (!Object.values(DocumentType).includes(type as DocumentType)) throw AppError.badRequest("Invalid document type"); if (!req.file) throw AppError.badRequest("No file uploaded"); const student = await Student.findOne({ _id: id, schoolId }); if (!student) throw AppError.notFound("Student not found"); const prepared = await prepareStudentDocument(req.file); const key = documentStorageKey(schoolId, id, type); const existingDocument = student.documents.find((document: any) => document.type === type); if (existingDocument?.url) { try { await preserveDocumentVersion(schoolId, id, existingDocument, "replacement"); } catch (error) { console.error("[Recovery] Replacement snapshot failed; R2 was not overwritten", error); throw AppError.internal("Unable to preserve the previous document version. Please try again."); } } const result = await uploadToR2(prepared.buffer, key, prepared.contentType); const documentMetadata = { originalName: req.file.originalname, mimeType: prepared.contentType, sizeBytes: prepared.buffer.length, uploadedAt: new Date() }; if (existingDocument) { existingDocument.url = result.key; existingDocument.originalName = documentMetadata.originalName; existingDocument.mimeType = documentMetadata.mimeType; existingDocument.sizeBytes = documentMetadata.sizeBytes; existingDocument.uploadedAt = documentMetadata.uploadedAt; } else student.documents.push({ _id: new mongoose.Types.ObjectId(), type: type as any, url: result.key, ...documentMetadata } as any); await student.save(); await createAuditLog({ userId: req.user!.userId, action: "UPLOAD_DOCUMENT", entity: "Student", entityId: student._id.toString(), after: { type, key: result.key, originalName: req.file.originalname, replaced: Boolean(existingDocument) } }); const uploadedDocument = student.documents.find((document: any) => document.type === type); res.status(201).json({ document: publicDocument(uploadedDocument) }); } catch (error) { next(error); } }
...
export async function exportStudents(req: Request, res: Response, next: NextFunction) { try { const { generateExcelFile } = await import("../services/excel.js"); const schoolId = getTenantId(req); const query: any = { schoolId }; if (req.user!.role === UserRole.TEACHER) { const classIds = await getTeacherClassIds(req); if (!classIds?.length) { res.status(200).send(await generateExcelFile([], "Students")); return; } query.classId = { $in: classIds }; } const students = await Student.find(query).select("-documents.url -documents.publicId").populate("classId sectionId").lean(); const buffer = await generateExcelFile(students, "Students"); res.setHeader("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"); res.setHeader("Content-Disposition", `attachment; filename=students-${new Date().toISOString().split("T")[0]}.xlsx`); res.send(buffer); } catch (error) { next(error); } }
