...
async function processEvent(event: NonNullable<Awaited<ReturnType<typeof claimNextEvent>>>) {
  ...
  if (!validRecipientIds.length) {
    await NotificationEvent.updateOne({ _id: event._id }, {
      $set: { status: "completed", processedAt: new Date(), lockedAt: undefined },
    });
    return;
  }
  ...
  const preferenceByUser = new Map(preferences.map((preference) => [preference.userId.toString(), preference.channels as NotificationChannel[]]));
  const mandatory = event.category === "system" || event.priority === "urgent";

  const operations = validRecipientIds
    .filter((recipientId) => mandatory || (preferenceByUser.get(recipientId.toString()) ?? ["in_app"]).includes("in_app"))
    .map((recipientId) => ({
      updateOne: {
        filter: {
          schoolId: event.schoolId,
          recipientId,
          idempotencyKey: `${event.idempotencyKey}:${recipientId.toString()}`,
        },
        update: {
          $setOnInsert: {
            schoolId: event.schoolId,
...
export async function startNotificationWorker() {
  ...
  const tick = async () => {
    ...
    running = true;
    try {
      while (await processOneNotificationEvent()) {
        // Drain currently available work, then yield to the event loop.
      }
    } finally {
      running = false;
    }
