...
export async function listAdmissionEnquiries(req: Request) {
  const schoolId = getTenantId(req); const stage = typeof req.query.stage === "string" ? req.query.stage : undefined; const search = typeof req.query.search === "string" ? req.query.search.trim() : ""; const filter: any = { schoolId };
...
export async function updateAdmissionEnquiry(req: Request, id: string, data: any) {
  const schoolId = getTenantId(req); const current = await AdmissionEnquiry.findOne({ _id: id, schoolId });
  ...
  if (current.stage === "converted" && data.stage && data.stage !== "converted") throw AppError.badRequest("A converted enquiry cannot move backwards");
