...
export async function runUpcomingCalendarNotifications(schoolId: Types.ObjectId, from = new Date(), horizonDays = 7) {
...
export async function runCalendarAutomation(schoolId: Types.ObjectId, runDate = new Date()) {
  return runIdempotentJob(schoolId, `calendar-upcoming:${runDate.toISOString().slice(0, 10)}`, () => runUpcomingCalendarNotifications(schoolId, runDate));
}
