...
export function generateIDCardPDF(student: Student & { classId: { displayName: string }; sectionId: { name: string } }): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    doc.text(`Name: ${student.firstName} ${student.lastName}`, 110, y);
    y += 20;
    doc.text(`Admission No: ${student.admissionNo}`, 110, y);
    y += 20;
    doc.text(`Class: ${student.classId.displayName} - ${student.sectionId.name}`, 110, y);
    y += 20;
    doc.text(`DOB: ${formatDate(student.dob)}`, 110, y);
    y += 20;
    doc.text(`Blood Group: ${student.bloodGroup || "N/A"}`, 110, y);
    y += 20;
...
export function generateTeacherIDCardPDF(teacher: Teacher): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    let y = 50;
    doc.fontSize(10).font("Helvetica-Bold");
    doc.text(`Name: ${teacher.firstName} ${teacher.lastName}`, 110, y);
    y += 20;
    doc.text(`Employee ID: ${teacher.employeeId}`, 110, y);
    y += 20;
    doc.text(`Qualification: ${teacher.qualification}`, 110, y);
    y += 20;
    doc.text(`Phone: ${teacher.phone}`, 110, y);
    y += 20;
