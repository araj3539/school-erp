...
function assertConfigured(): void {
  if (env.PAYMENT_PROVIDER !== "razorpay" || !env.RAZORPAY_KEY_ID || !env.RAZORPAY_KEY_SECRET) {
    throw AppError.serviceUnavailable("Online payment provider is not configured");
  }
}
...
export function isRazorpayConfigured(): boolean {
  return env.PAYMENT_PROVIDER === "razorpay" && Boolean(env.RAZORPAY_KEY_ID && env.RAZORPAY_KEY_SECRET);
}
...
export async function createRazorpayRefund(input: { paymentId: string; amount: number; idempotencyKey: string; receipt: string }): Promise<RazorpayRefundResponse> {
  return razorpayRequest<RazorpayRefundResponse>(`/payments/${encodeURIComponent(input.paymentId)}/refund`, {
    ...
    body: JSON.stringify({ amount: Math.round(input.amount * 100), receipt: input.receipt })
  });
}
