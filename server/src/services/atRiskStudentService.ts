...
export async function getAtRiskStudents(req: Request): Promise<{ academicYear: any; students: AtRiskStudent[] }> {
  ...
  for (const item of attendance as any[]) for (const record of item.records ?? []) {
    const id = record.studentId.toString(); const current = attendanceByStudent.get(id) ?? { total: 0, present: 0 }; current.total += 1; if (record.status === "present") current.present += 1; attendanceByStudent.set(id, current);
  }
  const resultsByStudent = new Map<string, any[]>(); for (const result of results as any[]) { const id = result.studentId.toString(); resultsByStudent.set(id, [...(resultsByStudent.get(id) ?? []), result]); }
  const behaviourByStudent = new Map<string, any[]>(); for (const item of behaviour as any[]) { const id = item.studentId.toString(); behaviourByStudent.set(id, [...(behaviourByStudent.get(id) ?? []), item]); }
  const scored: AtRiskStudent[] = students.map((student) => {
    const id = student._id.toString(); const factors: string[] = []; let score = 0; const attendanceStats = attendanceByStudent.get(id);
    if (attendanceStats?.total) { const rate = Math.round((attendanceStats.present / attendanceStats.total) * 100); if (rate < 75) { score += 30; factors.push(`Attendance ${rate}% (below 75%)`); } else if (rate < 85) { score += 15; factors.push(`Attendance ${rate}% (below 85%)`); } }
    const studentResults = resultsByStudent.get(id) ?? []; if (studentResults.some((result) => result.result === "fail")) { score += 25; factors.push("At least one published failed result"); }
    ...
    return { student, score: clamp(score), level: score >= 50 ? "intervention" : "watch", factors };
