...
export async function getAtRiskStudents(req: Request): Promise<{ academicYear: any; students: AtRiskStudent[] }> {
  ...
  for (const item of attendance as any[]) for (const record of item.records ?? []) { const id = record.studentId.toString(); const current = attendanceByStudent.get(id) ?? { total: 0, present: 0 }; current.total += 1; if (record.status === "present") current.present += 1; attendanceByStudent.set(id, current); }
