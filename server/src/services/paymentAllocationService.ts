...
export async function allocatePaymentToItems(schoolId: string, paymentId: string, actorId: string, session?: mongoose.ClientSession) {
  const run = async (tx: mongoose.ClientSession) => {
    ...
    for (const item of items) {
      ...
      await PaymentAllocation.create([{ schoolId, paymentId: payment._id, feeItemId: item._id, amount, type: "allocation", createdBy: new mongoose.Types.ObjectId(actorId) }], { session: tx });
...
export async function reversePaymentAllocations(schoolId: string, paymentId: string, reversalId: string, amount: number, actorId: string, session: mongoose.ClientSession) {
  ...
  for (const allocation of allocations) {
    if (remaining <= 0.009) break;
    const reversed = await PaymentAllocation.aggregate([{ $match: { schoolId: new mongoose.Types.ObjectId(schoolId), originalAllocationId: allocation._id, type: "reversal" } }, { $group: { _id: null, total: { $sum: "$amount" } } }]).session(session);
    const available = allocation.amount - (reversed[0]?.total ?? 0);
    ...
    await PaymentAllocation.create([{ schoolId, paymentId: payment._id, feeItemId: item._id, amount: part, type: "reversal", reversalId: new mongoose.Types.ObjectId(reversalId), originalAllocationId: allocation._id, createdBy: new mongoose.Types.ObjectId(actorId) }], { session });
  ...
  return { reversed: amount - remaining, unallocatedReversal: remaining, allocations: rows };
}
