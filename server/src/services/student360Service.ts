...
export async function getStudent360(req: Request, studentId: string) {
  ...
  if (!Types.ObjectId.isValid(studentId)) throw AppError.badRequest("Invalid student id");
  ...
  if (req.user!.role === UserRole.STUDENT) access.userId = req.user!.userId;
    ...
    const Teacher = (await import("../models/Teacher.js")).Teacher;
    const teacher = await Teacher.findOne({ userId: req.user!.userId, schoolId }).select("classTeacherOf").lean();
    if (!teacher?.classTeacherOf?.some((id) => id.toString() === student.classId?.toString())) throw AppError.forbidden("You are not assigned to this student's class");
  }

  const student = await Student.findOne(access).lean();
  if (!student) throw AppError.notFound("Student not found");

  const [parents, siblings, lifecycle, attendance, results, homework] = await Promise.all([
  ...
  const attendanceSummary = attendance.reduce((acc, day: any) => {
    const record = day.records.find((r: any) => r.studentId.toString() === student._id.toString());
  ...
  return {
    student: { _id: student._id, admissionNo: student.admissionNo, firstName: student.firstName, lastName: student.lastName, dob: student.dob, gender: student.gender, status: student.status, classId: student.classId, sectionId: student.sectionId, admissionDate: student.admissionDate, phone: student.phone, guardianPhone: student.guardianPhone },
    parents,
    siblings: siblingStudents,
    attendance: { summary: { ...attendanceSummary, percentage: attendancePercentage }, recent: attendance.map((day: any) => { const r = day.records.find((x: any) => x.studentId.toString() === student._id.toString()); return { date: day.date, status: r?.status, remark: r?.remark, lifecycle: day.lifecycle }; }) },
    academics: { results, averagePercentage, publishedResultCount: results.length },
    homework: { items: homework, upcomingCount: pendingHomework },
    lifecycle,
  };
}
