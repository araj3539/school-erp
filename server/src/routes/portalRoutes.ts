...
router.get("/teacher/workspace", requirePermission("timetable:read:own"), (req, res, next) => req.user!.role !== UserRole.TEACHER ? res.status(403).json({ error: "Teacher workspace is not available for this role" }) : getTeacherWorkspace(req, res, next));
