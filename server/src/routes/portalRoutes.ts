...
router.get("/attendance", (req, res, next) => {
  if (req.user!.role === UserRole.STUDENT) return requirePermission("attendance:read:own")(req, res, next);
  if (req.user!.role === UserRole.PARENT) return requirePermission("attendance:read:child")(req, res, next);
  return res.status(403).json({ error: "Portal attendance is not available for this role" });
}, getPortalAttendance);
router.get("/results", (req, res, next) => {
  if (req.user!.role === UserRole.STUDENT) return requirePermission("results:read:own")(req, res, next);
  if (req.user!.role === UserRole.PARENT) return requirePermission("results:read:child")(req, res, next);
  return res.status(403).json({ error: "Portal results are not available for this role" });
}, getPortalResults);
