...
describe("Phase 5 academic result models", () => {
  it("rejects overlapping grade ranges", async () => {
    const exam = new Exam({ startDate: "2026-09-01", endDate: "2026-09-02", subjects: [{ subjectId: "507f1f77bcf86cd799439011", maxMarks: 100, passMarks: 40 }], gradeRules: [{ grade: "A", minPercentage: 70, maxPercentage: 100 }, { grade: "B", minPercentage: 50, maxPercentage: 80 }, { grade: "F", minPercentage: 0, maxPercentage: 60 }] });
  ...
  it("rejects incomplete grade coverage", async () => {
    const exam = new Exam({ startDate: "2026-09-01", endDate: "2026-09-02", subjects: [{ subjectId: "507f1f77bcf86cd799439011", maxMarks: 100, passMarks: 40 }], gradeRules: [{ grade: "A", minPercentage: 80, maxPercentage: 100 }] });
    await expect(exam.validate()).rejects.toThrow("complete 0-100 percentage scale");
  });

  it("rejects duplicate subjects in an exam", async () => {
    const exam = new Exam({ startDate: "2026-09-01", endDate: "2026-09-02", subjects: [{ subjectId: "507f1f77bcf86cd799439011", maxMarks: 100, passMarks: 40 }, { subjectId: "507f1f77bcf86cd799439011", maxMarks: 50, passMarks: 20 }], gradeRules: [{ grade: "A", minPercentage: 0, maxPercentage: 100 }] });
    await expect(exam.validate()).rejects.toThrow("same subject more than once");
  });
