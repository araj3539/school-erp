...
AttendanceSchema.pre("save", function (next) {
  if (!this.isNew && this.$isDeleted?.()) return next();
  if (!this.isNew && this.get("lifecycle") === "LOCKED" && this.isModified("records")) return next(new Error("Locked attendance cannot be edited without an approved correction"));
  next();
});
