...
export interface IPaymentAllocation extends Document {
  ...
  originalAllocationId?: Types.ObjectId;
  createdBy: Types.ObjectId;
  createdAt: Date;
}
const schema = new Schema<IPaymentAllocation>({
  schoolId: { type: Schema.Types.ObjectId, ref: "School", required: true }, paymentId: { type: Schema.Types.ObjectId, ref: "Payment", required: true }, feeItemId: { type: Schema.Types.ObjectId, ref: "FeeItem", required: true }, amount: { type: Number, required: true, min: 0.01 }, type: { type: String, enum: ["allocation", "reversal"], required: true }, reversalId: { type: Schema.Types.ObjectId, ref: "PaymentReversal" }, originalAllocationId: { type: Schema.Types.ObjectId, ref: "PaymentAllocation" }, createdBy: { type: Schema.Types.ObjectId, ref: "User", required: true }
}, { timestamps: { createdAt: true, updatedAt: false } });
schema.pre("validate", async function () { const [payment, item] = await Promise.all([Payment.exists({ _id: this.paymentId, schoolId: this.schoolId }), FeeItem.exists({ _id: this.feeItemId, schoolId: this.schoolId })]); if (!payment) throw new Error("Payment allocation must belong to the same school as its payment"); if (!item) throw new Error("Payment allocation fee item must belong to the same school"); if (this.type === "reversal" && !this.reversalId) throw new Error("Reversal allocation requires a reversal reference"); if (this.type === "reversal" && !this.originalAllocationId) throw new Error("Reversal allocation requires its original allocation reference"); });
...
schema.index({ schoolId: 1, originalAllocationId: 1 });
