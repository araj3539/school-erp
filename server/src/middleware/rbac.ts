import { Request, Response, NextFunction } from "express";
import { ROLE_PERMISSIONS } from "@school-erp/shared";
import mongoose from "mongoose";
import { School } from "../models/index.js";

export async function requireActiveTenant(req: Request, res: Response, next: NextFunction): Promise<void> {
  const schoolId = req.user?.schoolId || (req.user?.role === "super_admin" ? req.get("X-School-Id")?.trim() : undefined);
  if (!schoolId || !mongoose.isValidObjectId(schoolId)) { res.status(403).json({ error: "Active school context is required" }); return; }
  try {
    const school = await School.findById(schoolId).select("tenantStatus").lean();
    if (!school) { res.status(404).json({ error: "School not found" }); return; }
    if ((school.tenantStatus ?? "active") !== "active") { res.status(403).json({ error: "School access is unavailable while this tenant is not active" }); return; }
    next();
  } catch (error) { next(error); }
}

export function requirePermission(permission: string) {
  return (req: Request, res: Response, next: NextFunction): void => {
    if (!req.user) { res.status(401).json({ error: "Authentication required" }); return; }
    const userPermissions = ROLE_PERMISSIONS[req.user.role as keyof typeof ROLE_PERMISSIONS] || [];
    if (userPermissions.includes("*") || userPermissions.includes(permission)) { next(); return; }
    res.status(403).json({ error: `Permission required: ${permission}` });
  };
}

export function requireAnyPermission(...permissions: string[]) {
  return (req: Request, res: Response, next: NextFunction): void => {
    if (!req.user) { res.status(401).json({ error: "Authentication required" }); return; }
    const userPermissions = ROLE_PERMISSIONS[req.user.role as keyof typeof ROLE_PERMISSIONS] || [];
    if (userPermissions.includes("*") || permissions.some((p) => userPermissions.includes(p))) { next(); return; }
    res.status(403).json({ error: `One of permissions required: ${permissions.join(", ")}` });
  };
}

export function requireOwnership(getResourceUserId: (req: Request) => string) {
  return (req: Request, res: Response, next: NextFunction): void => {
    if (!req.user) { res.status(401).json({ error: "Authentication required" }); return; }
    const resourceUserId = getResourceUserId(req);
    if (req.user.role === "super_admin" || req.user.userId === resourceUserId) { next(); return; }
    res.status(403).json({ error: "Access denied" });
  };
}
