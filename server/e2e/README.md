...
Use `npm run seed:e2e` from `server/` only against a dedicated test or staging MongoDB database.

Required environment:

- `E2E_FIXTURE_SEED_ENABLED=true`
- `E2E_MONGODB_URI` pointing explicitly to the test/staging database; the application's `MONGODB_URI` is intentionally not accepted
...
**Safety:** the seeder refuses to run when `NODE_ENV=production` or when the explicit enable flag is absent, and it requires a separate `E2E_MONGODB_URI` rather than the application's production database variable. Never point it at the production database.
