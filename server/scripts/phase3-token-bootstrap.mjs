...
if (!apiUrl || !password) {
  console.error("Phase 3 token bootstrap requires E2E_API_URL and E2E_FIXTURE_PASSWORD");
  process.exit(2);
}
...
async function login(email) {
  ...
  if (!response.ok) throw new Error(`Login failed for ${email}: ${response.status} ${JSON.stringify(body)}`);
...
try {
  const outputPath = resolve(tmpdir(), `school-erp-phase3-${randomUUID()}.json`);
  const tokens = {};
  for (const [role, email] of Object.entries(fixtures)) tokens[role] = { accessToken: await login(email) };
