...
let tokens;
try { tokens = JSON.parse(readFileSync(tokenFile, "utf8")); } finally { try { unlinkSync(tokenFile); } catch {} }

const env = { ...process.env, E2E_PRINCIPAL_A_ACCESS_TOKEN: tokens.principalA.accessToken, E2E_TEACHER_A_ACCESS_TOKEN: tokens.teacherA.accessToken };
const suites = [
  ...
  ["teachers", ["run", "test:e2e:phase3:teachers"]],
  ["attendance-report", ["run", "test:e2e:phase3:attendance-report"]],
  ["dashboard", ["run", "test:e2e:phase3:dashboard"]],
];
