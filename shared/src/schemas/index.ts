...
export const UpdateSchoolSettingsSchema = z.object({ name: z.string().min(2).max(100).optional(), logo: z.string().url().nullable().optional(), address: z.string().max(500).optional(), phone: z.string().max(20).optional(), email: z.string().email().optional(), session: z.string().max(20).optional(), settings: z.record(z.unknown()).optional() }).refine((value) => Object.keys(value).length > 0, "At least one school setting must be supplied");
...
export const BulkAttendanceSchema = z.object({ entries: z.array(BulkAttendanceEntrySchema).min(1).max(31) }).superRefine((value, ctx) => {
  ...
  value.entries.forEach((entry, index) => { const key = `${entry.date}:${entry.classId}:${entry.sectionId}`; if (keys.has(key)) ctx.addIssue({ code: z.ZodIssueCode.custom, path: ["entries", index, "date"], message: "Duplicate attendance day for class and section" }); keys.add(key); });
});
