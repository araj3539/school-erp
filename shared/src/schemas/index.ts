...
const AttendanceRecordInputSchema = z.object({ studentId: ObjectIdSchema, status: z.nativeEnum(AttendanceStatus), remark: z.string().max(200).optional() });
export const AttendanceSchema = z.object({ _id: ObjectIdSchema.optional(), date: DateOnlySchema, classId: ObjectIdSchema, sectionId: ObjectIdSchema, records: z.array(AttendanceRecordInputSchema), markedBy: ObjectIdSchema, createdAt: z.string().datetime().optional(), updatedAt: z.string().datetime().optional() });
export const CreateAttendanceSchema = AttendanceSchema.omit({ _id: true, createdAt: true, updatedAt: true });
export const UpdateAttendanceSchema = CreateAttendanceSchema.partial();
export const MarkAttendanceSchema = z.object({ date: DateOnlySchema, classId: ObjectIdSchema, sectionId: ObjectIdSchema, records: z.array(AttendanceRecordInputSchema).min(1).max(200) });
export const BulkAttendanceEntrySchema = MarkAttendanceSchema;
export const BulkAttendanceSchema = z.object({ entries: z.array(BulkAttendanceEntrySchema).min(1).max(31) }).superRefine((value, ctx) => {
  const totalRecords = value.entries.reduce((sum, entry) => sum + entry.records.length, 0);
  if (totalRecords > 5000) ctx.addIssue({ code: z.ZodIssueCode.custom, path: ["entries"], message: "Bulk attendance cannot contain more than 5000 student records" });
  const keys = new Set<string>();
  value.entries.forEach((entry, index) => {
    const key = `${entry.date}:${entry.classId}:${entry.sectionId}`;
    if (keys.has(key)) ctx.addIssue({ code: z.ZodIssueCode.custom, path: ["entries", index, "date"], message: "Duplicate attendance day for class and section" });
    keys.add(key);
  });
});
