...
function idOf(item: unknown): string {
  ...
  const value = (item as Record<string, unknown>)._id;
...
function ChoiceRow({ label, selected, onPress, testID }: { label: string; selected: boolean; onPress: () => void; testID: string }) {
  return <Pressable
...
export function TeacherScreen() {
  ...
  const load = useCallback(async () => {
    ...
    setRefreshing(true); setError(null); setSavedMessage(null);
    try {
      const nextData = await api.getWorkspace();
      if (sequence === loadSequence.current) setData(nextData);
    } catch (cause) {
      if (sequence === loadSequence.current) setError(cause instanceof ApiError ? cause.message : "Unable to load the teacher workspace.");
    } finally {
  ...
  useEffect(() => {
    if (!selectedClassId && classOptions.length) setSelectedClassId(idOf(classOptions[0]));
    if (selectedClassId && !classOptions.some((item) => idOf(item) === selectedClassId)) setSelectedClassId(classOptions.length ? idOf(classOptions[0]) : "");
  }, [classOptions, selectedClassId]);

  useEffect(() => {
    if (!sectionOptions.some((item) => idOf(item) === selectedSectionId)) setSelectedSectionId(sectionOptions.length ? idOf(sectionOptions[0]) : "");
  }, [sectionOptions, selectedSectionId]);

  useEffect(() => {
    const next: Record<string, Status> = {};
    studentOptions.forEach((student) => { next[idOf(student)] = "present"; });
    const records = existingAttendance?.records;
    if (Array.isArray(records)) records.forEach((record) => { if (record && typeof record === "object") { const studentId = idOf((record as Record<string, unknown>).studentId); const status = (record as Record<string, unknown>).status; if (studentId && typeof status === "string" && STATUSES.some((item) => item.value === status)) next[studentId] = status as Status; } });
    setStatuses(next);
  }, [studentOptions, existingAttendance]);
  ...
  const saveAttendance = useCallback(async () => {
    ...
    try {
      await api.markAttendance({
        ...
        records: studentOptions.map((student) => ({ studentId: idOf(student), status: statuses[idOf(student)] ?? "present" })),
      });
      setSavedMessage("Attendance saved successfully.");
      await load();
    } catch (cause) {
      setError(cause instanceof ApiError ? cause.message : "Unable to save attendance.");
  ...
  >
    ...
    {savedMessage ? <Text accessibilityLiveRegion="polite" style={styles.success}>{savedMessage}</Text> : null}

    <Section title="Mark attendance">
      <Text style={styles.helper}>Choose an assigned class and section, then set each student’s status.</Text>
      <Text style={styles.controlLabel}>Class</Text>
      <View style={styles.choiceWrap}>
        {classOptions.map((item) => <ChoiceRow key={idOf(item)} label={value(item, "displayName", value(item, "name", "Class"))} selected={idOf(item) === selectedClassId} onPress={() => { setSelectedClassId(idOf(item)); setSelectedSectionId(""); setSavedMessage(null); }} testID={`teacher-class-${idOf(item)}`} />)}
      </View>
      {selectedClassId ? <>
        <Text style={styles.controlLabel}>Section</Text>
        <View style={styles.choiceWrap}>
          {sectionOptions.map((item) => <ChoiceRow key={idOf(item)} label={value(item, "name", "Section")} selected={idOf(item) === selectedSectionId} onPress={() => { setSelectedSectionId(idOf(item)); setSavedMessage(null); }} testID={`teacher-section-${idOf(item)}`} />)}
        </View>
      </> : null}
      {!sectionOptions.length && selectedClassId ? <EmptyState message="No sections are assigned to this class." /> : null}
      {selectedSectionId ? <>
        <Text style={styles.dateLabel}>Attendance date: {data.date}</Text>
        ...
        {studentOptions.length ? studentOptions.map((student) => {
          const studentId = idOf(student);
          return <View key={studentId} style={styles.studentCard}>
            <Text style={styles.studentName}>{value(student, "firstName")} {value(student, "lastName")}</Text>
            <Text style={styles.studentMeta}>{value(student, "admissionNo")}</Text>
            <View style={styles.statusWrap}>
              {STATUSES.map((status) => <ChoiceRow key={status.value} label={status.label} selected={statuses[studentId] === status.value} onPress={() => { if (!hasExistingAttendance) setStatuses((current) => ({ ...current, [studentId]: status.value })); }} testID={`teacher-attendance-${studentId}-${status.value}`} />)}
            </View>
          </View>;
        }) : <EmptyState message="No active students are assigned to this section." />}
        <Pressable
          accessibilityRole="button"
          accessibilityLabel="Save attendance"
          accessibilityState={{ disabled: saving || hasExistingAttendance || !studentOptions.length }}
          disabled={saving || hasExistingAttendance || !studentOptions.length || !data.permissions.canMarkAttendance}
          onPress={() => void saveAttendance()}
          style={({ pressed }) => [styles.saveButton, (pressed || saving) && styles.pressed, (hasExistingAttendance || !studentOptions.length) && styles.disabledButton]}
        ><Text style={styles.saveText}>{saving ? "Saving…" : "Save attendance"}</Text></Pressable>
      </> : null}
    </Section>
...
const styles = StyleSheet.create({
  ...
  helper: { color: mobileTheme.colors.textMuted, lineHeight: 21, marginBottom: 10 },
  controlLabel: { fontSize: 14, fontWeight: "700", color: mobileTheme.colors.text, marginTop: 8, marginBottom: 6 },
  choiceWrap: { flexDirection: "row", flexWrap: "wrap", gap: 8 },
  choice: { minHeight: mobileTheme.touchTarget, minWidth: 74, justifyContent: "center", alignItems: "center", paddingHorizontal: 12, borderWidth: 1, borderColor: mobileTheme.colors.border, borderRadius: mobileTheme.radius.sm, backgroundColor: mobileTheme.colors.surface },
  choiceSelected: { borderColor: mobileTheme.colors.text, backgroundColor: mobileTheme.colors.text },
  choiceText: { color: mobileTheme.colors.text, fontWeight: "600", fontSize: 13 },
  choiceTextSelected: { color: mobileTheme.colors.surface },
  dateLabel: { marginTop: 14, fontWeight: "700", color: mobileTheme.colors.text },
  studentCard: { marginTop: 12, padding: 12, borderRadius: mobileTheme.radius.sm, backgroundColor: mobileTheme.colors.background, borderWidth: 1, borderColor: mobileTheme.colors.border },
  studentName: { fontWeight: "700", color: mobileTheme.colors.text, fontSize: 16 },
  studentMeta: { color: mobileTheme.colors.textMuted, marginTop: 2, marginBottom: 8 },
  statusWrap: { flexDirection: "row", flexWrap: "wrap", gap: 6 },
  readOnly: { marginTop: 12, padding: 12, borderRadius: mobileTheme.radius.sm, backgroundColor: mobileTheme.colors.warningBackground },
  readOnlyTitle: { fontWeight: "700", color: mobileTheme.colors.text, marginBottom: 4 },
  saveButton: { minHeight: mobileTheme.touchTarget, marginTop: 16, borderRadius: mobileTheme.radius.sm, backgroundColor: mobileTheme.colors.text, justifyContent: "center", alignItems: "center", paddingHorizontal: 16 },
  saveText: { color: mobileTheme.colors.surface, fontWeight: "700", fontSize: 16 },
  disabledButton: { opacity: 0.5 },
  success: { color: mobileTheme.colors.successText, marginTop: 10, fontWeight: "600" },
