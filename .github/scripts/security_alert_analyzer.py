...
def validate_analysis(value: dict[str, Any]) -> dict[str, Any]:
    ...
    if value["verdict"] not in {"true_positive", "false_positive", "needs_investigation"}:
        raise APIError("Invalid verdict")
    if not isinstance(value["confidence"], (int, float)) or not 0 <= value["confidence"] <= 1:
        raise APIError("Invalid confidence")
    return value
...
def nvidia_json(instructions: str, evidence: dict[str, Any], api_key: str) -> dict[str, Any]:
    ...
    payload = {
        "model": MODEL,
        "messages": [
            {"role": "system", "content": "You are a senior application security reviewer. Output JSON only."},
...
",
    ...
    try:
        ...
        raise APIError(f"NVIDIA response missing assistant content: {json.dumps(result)[:2000]}") from exc
...
def github_alerts(
    ...
    for page in range(1, 101):
        ...
        if not isinstance(items, list):
            raise APIError("Unexpected code-scanning alerts response")
        for item in items:
...
def source_context(source: str, start: int, end: int) -> str:
    ...
    start, end = max(1, start - SOURCE_CONTEXT_LINES), min(len(lines), end + SOURCE_CONTEXT_LINES)
...
def build_rollup(report: list[dict[str, Any]], failures: list[dict[str, Any]]) -> dict[str, dict[str, int]]:
    return {
        "verdict": dict(Counter(item["analysis"]["verdict"] for item in report)),
        "severity": dict(Counter(item["analysis"]["severity"] for item in report)),
        "exploitability": dict(Counter(item["analysis"]["exploitability"] for item in report)),
        "tenant_boundary_risk": dict(Counter(item["analysis"]["tenant_boundary_risk"] for item in report)),
        "human_review_required": {"yes": sum(bool(item["analysis"]["requires_human_review"]) for item in report)},
        "analysis_failures": {"count": len(failures)},
    }
...
def main() -> int:
    ...
    try:
        alert_numbers = {int(x) for x in raw_numbers}
    except ValueError as exc:
        raise APIError("ALERT_NUMBERS must be a comma-separated list of integers") from exc
    if not github_token:
        raise APIError("GITHUB_TOKEN is required")
    if not nvidia_key:
        raise APIError("NVIDIA_API_KEY GitHub Actions secret is required")

    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    alerts = github_alerts(github_token, target_ref, alert_state, updated_after, alert_numbers)
    ...
    for index, alert in enumerate(alerts, 1):
        ...
        end = int(location.get("end_line") or location.get("start_line") or 1)
        commit = instance.get("commit_sha") or ""
        source, context_error = "", None
        if path and commit:
            try:
                source = github_file(github_token, path, commit)
    ...
    rollup = build_rollup(report, failures)
    result = {
        "schema_version": 2,
        "repository": f"{OWNER}/{REPO}",
        "target_ref": target_ref,
        ...
        "alert_numbers": sorted(alert_numbers),
        "model": MODEL,
    ...
    (OUTPUT_DIR / "security-triage-report.json").write_text(json.dumps(result, indent=2, ensure_ascii=False), encoding="utf-8")

    markdown = [
        "# Security Triage Report",
        "",
        "> AI-assisted, read-only triage of GitHub Code Scanning alerts. This report does not dismiss, modify, or suppress alerts.",
        "",
        "## Run summary",
        "",
        ...
        f"- Updated after: `{updated_after or 'none'}`",
        f"- Explicit alert numbers: `{','.join(map(str, sorted(alert_numbers))) if alert_numbers else 'none'}`",
        f"- Model: `{MODEL}`",
        f"- Generated: `{result['generated_at']}`",
        f"- Alerts selected: **{len(alerts)}**",
        f"- Analyses completed: **{len(report)}**",
        f"- Analysis failures: **{len(failures)}**",
        "- Mode: **read-only**",
        "",
        "## Triage roll-up",
        "",
        "| Dimension | Result |",
        "| --- | --- |",
        f"| Verdicts | `{json.dumps(rollup['verdict'], sort_keys=True)}` |",
        f"| Severity | `{json.dumps(rollup['severity'], sort_keys=True)}` |",
        f"| Exploitability | `{json.dumps(rollup['exploitability'], sort_keys=True)}` |",
        f"| Tenant-boundary risk | `{json.dumps(rollup['tenant_boundary_risk'], sort_keys=True)}` |",
        f"| Human review required | `{rollup['human_review_required']['yes']}` |",
        "",
    ]

    for item in report:
        analysis = item["analysis"]
        alert_link = f" — [GitHub alert]({item['html_url']})" if item.get("html_url") else ""
        markdown += [
            f"## Alert #{item['alert_number']} — `{item['rule_id']}`{alert_link}",
            "",
